...
- **G3 — Crisis path is LLM-free and simulated-only.** `escalate_to_human` is a static tool: deterministic canned return, no model call, no transfer, no TwiML/bridge change; verbs-after-`</Connect>` stays designed out. **SIMULATION-ONLY guarantee (human directive, 2026-07-19): no caller is ever transferred, forwarded, conferenced, or connected to any real line — crisis or otherwise. The system has no transfer capability by design (`live_transfer:false` everywhere) and no demo task may add one. Real phone numbers are SPOKEN as information only.** The crisis resource numbers are spoken verbatim and must be **byte-identical wherever they appear**: Counseling Center **(661) 654-3366** (after hours press 2), **988** (call or text), UPD **(661) 654-2111** / 911, operator **(661) 654-2782** — surfaces: Spec 02 R3 payload, Spec 04 corpus §9, Spec 06 deep-dive §3 and email; the Spec 01 R3 Safety backup carries the three crisis numbers only (Counseling, 988, UPD/911 — NOT the operator line, per the D15 adjudication). Byte-identity is checked where-present per surface. [findings/13 claims 20–22; findings/17 §4.5]
...
**D15 — Operator number not in the persona (HUMAN-DECIDED 2026-07-19).** The cross-plan review found A5 (as originally written) required `(661) 654-2782` in `src/gateway.ts`, while Spec 01 R3's Safety backup deliberately carries only the three crisis numbers. The human chose Spec 01: the persona keeps Counseling/988/UPD only; the operator number's surfaces are `src/mcp-server.ts` (route_call/escalate payloads), the corpus, and the docs. A5 and G3 amended accordingly (byte-identity where-present). The human simultaneously reaffirmed the G3 SIMULATION-ONLY guarantee: real numbers are information spoken aloud, never transfer targets — no demo task may introduce any forwarding capability.
...
Numbering: origin spec → issue → **disposition** (adjudications D1–D15 referenced from §5).
...
**Produces (for the orchestrator and all implementer sub-agents):** the canonical numbering (§1); the folder contract incl. `docs/demo/plans/LEDGER.md` (§2); global constraints G1–G14 (§3); the frozen interface table (§4); binding adjudications D1–D15 (§5); the DA/DB/DC/DD wave plan with merge points M-A/M-B/M-C and human queue H1–H3 (§6); spikes DS-1…DS-5 (§7); test rules incl. KF-1 (§8).
...
- **A5** (crisis-number identity, G3, amended per D15): `(661) 654-3366`, `988`, and `(661) 654-2111` each appear in `src/gateway.ts`, `src/mcp-server.ts`, and `assets/csub-corpus.md` after M-B; `(661) 654-2782` appears in `src/mcp-server.ts` and `assets/csub-corpus.md` (NOT required in `src/gateway.ts`); all four appear in `docs/demo/MCP-SERVER-DEEP-DIVE.md` after DC2 — byte-identical formatting wherever present.
