...
| DEV-03 | 2026-07-19 | H1 live round 2 | Knowledge cold-start hit 3500ms abort (2/3 calls) + comprehensive answers blew 150-token cap (AI_NoObjectGeneratedError); 31924 recurred (5/12 calls incl. a greeting-only call) with burst hypothesis REFUTED by outbound-burst data; 31924-killed calls left sockets half-open 30s-2min | Deployed: knowledge tuning (timeout 4500ms, cap 400, brevity guard), proactive teardown on stream-error callback, Twilio-leg ping/pong heartbeat (findings/18 addendum: transport flakiness, medium-high conf). Rio voice fix confirmed live |
| DEV-04 | 2026-07-19 | H1 live round 3 (0058bfb) | Verification round after DEV-03 fix bundle: 3 calls, 16 turns, ~6 min. Knowledge tool 6/6 ok incl. first-call-after-boot at 1508.8ms (no cold-start abort); comprehensive "tell me everything" answered fully twice (147/178 output tokens, brevity guard shaping correctly); not_found → route_call chain correct; heartbeat clean on all calls (13/13, 45/45, 10/10 pongs; maxRtt 15–18ms; zero pong-missed warns); all closes 1000 caller-hangup with immediate teardown, zero phantom post-call logs; zero 31924 in the round | DEV-03 items (a) knowledge tuning and (c) zombie sockets CONFIRMED FIXED. (b) 31924 did not recur (0/3 vs prior 5/12) — teardown+heartbeat discriminators armed but unexercised; treat as watch item, not blocker. H1 → PASS pending Kevin's call |
| DEV-05 | 2026-07-19 | H1 live round 4 (0058bfb, multi-caller) | First concurrent-caller test: 3× 31924 in a 20s window (CA785c04 3.3s solo-ish; CAc32b4e 14.7s + CA40f858 8.8s CONCURRENT, failing 4ms apart per Twilio's own timestamps); zero failures in 8 surrounding solo calls. Heartbeat: pongs clean (14-15ms RTT) right up to the instant of failure on both — findings/18 claim-22 discriminator fired AGAINST H2' (gradual per-connection flakiness) | findings/18 Addendum 2 (claims 24-29): verdict revised to H5 — abrupt correlated reset at shared Railway edge (high conf); app changes NOT indicated. DEV-03 mitigations verified under real failure: instant teardown, no zombies, heartbeat summaries emitted. Remediation = infrastructure decision (Railway ticket w/ evidence pack + reproducer / region change / move WS endpoint) — human decision pending; gates announcement email |
| DEV-06 | 2026-07-19 | single-caller robustness (post-DEV-05) | Human directive: stay on Railway for now, caller must never be hung up on by an infra drop (Fly.io migration deferred, explicitly NOT planned yet). This supersedes the base-build design lock "NO verbs after </Connect>, NO action attribute" (old twiml.ts:164, FR-7 clean-hangup arm) | Implemented via worktree implementer + task review (Approved): `<Connect action>` reconnect flow — end-reason registry (src/reconnect.ts, abnormal vs expected, 5-min TTL sweep), /twiml-action (G8 signature-validated) re-issues `<Connect><Stream>` on abnormal end, STREAM_RECONNECT_MAX default 2, RECONNECT_GREETING_INSTRUCTIONS ("we lost the line for a moment"); + persona line: never restart greeting after barge-in. 472/472 (20 new tests), typecheck clean, merged 7fb09a2. Review Minors (accepted, live-verify): (1) statusCallback-vs-action race on no-close-frame kills → worst case = today's behavior (call ends, no loop); (2) attempts cap resets per 5-min TTL window (bounded, no loop); (3) 403 content-type cosmetic. Live verification pending: single caller + observe `stream-reconnect` on any 31924 |
