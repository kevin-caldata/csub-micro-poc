...
import { sessions, type SessionHandle } from './state.js';
...
describe('shutdown — drain before close (Spec 02 R8, A7/A8)', () => {
  it('A7 gate + drain-before-close: gates /twiml, /health, and new WS upgrades; exempts /stream-status; leaves the already-open WS alone; resolves quickly once sessions empties; no straggler teardown', async () => {
    ...
    assert.equal(twimlRes.statusCode, 503);
    ...
    assert.equal(streamStatusRes.statusCode, 204);

    await assert.rejects(() => app.injectWS('/test-ws')); // non-101 refusal during drain

    assert.equal(closed, false); // already-open WS not severed while draining
    ...
    assert.equal(teardownCalls, 0); // straggler teardown never needed
  ...
  it('A7 straggler sweep: teardown(\'server shutdown\') invoked when sessions never empties; exits by ~deadline', async () => {
    ...
    assert.equal(lastReason, 'server shutdown');
    assert.deepEqual(exitCalls, [0]);
    assert.ok(elapsed < 2000, `expected shutdown to resolve well under 2000ms, got ${elapsed}ms`);
  });
