...
import { MULAW_DEC } from '../src/dsp.js';
...
describe('assets/fallback-apology.ulaw (Spec 09 A5)', () => {
  it('exists and is a regular file with nonzero size', () => {
    ...
    expect(stat.isFile(), `${ASSET_PATH} should be a regular file`).toBeTruthy();
    expect(stat.size > 0, `${ASSET_PATH} should be nonempty`).toBeTruthy();
  });
  ...
  it('is headerless: first 4 bytes are NOT the RIFF container magic', () => {
    ...
    expect(first4, 'asset must be raw mu-law, not a WAV container').not.toBe('RIFF');
  });

  it('byte length is within 24,000-56,000 bytes (~3-7 s at 8000 B/s)', () => {
    const stat = statSync(ASSET_PATH);
    expect(stat.size >= 24_000 && stat.size <= 56_000, `expected 24000-56000 bytes, got ${stat.size}`).toBeTruthy();
  });

  it('byte length equals the raw file size exactly (no trailing container chunk)', () => {
    const buf = readFileSync(ASSET_PATH);
    const stat = statSync(ASSET_PATH);
    expect(buf.length).toBe(stat.size);
  ...
  it('round-trips through MULAW_DEC without throwing and stays in PCM16 range', () => {
    ...
    for (let i = 0; i < buf.length; i += step) {
      ...
      expect(Number.isInteger(pcm), `decoded sample at byte ${i} should be an integer`).toBeTruthy();
      expect(pcm >= -32768 && pcm <= 32767, `decoded sample at byte ${i} out of PCM16 range`).toBeTruthy();
    }
  });
});
